...
class CallHelperTest
{
    ...
    {
        $name = env('SKIJASI_USER_NAME', 'skijasi.test');
        $username = $name;
        $email = "{$name}@test.com";
    ...
    {
        $api_prefix = config('skijasi.api_route_prefix', 'skijasi-api');
    ...
    {
        return 'skijasi-api/v1/auth/'.$path;
    }

    public static function getContentApiV1($path)
    {
        return 'skijasi-api/module/content/v1'.$path;
    }

    public static function getApiV1($path)
    {
        return 'skijasi-api/module/post/v1'.$path;
    }
}
