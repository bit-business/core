...
namespace NadzorServera\Skijasi\Traits;
...
trait FileHandler
{
    public function handleUploadFiles($files, $data_type = null, $custom_path = null)
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_array($file) && array_key_exists('base64', $file)) {
                ...
                Storage::disk(config('skijasi.storage.disk', 'public'))->put($filepath.$filename, $decoded_file);
    ...
    {
        return Storage::disk(config('skijasi.storage.disk', 'public'))->delete($file);
    }

    public function handleDownloadFile($file)
    {
        return Storage::disk(config('skijasi.storage.disk', 'public'))->download($file);
    }

    public function handlePathFile($file)
    {
        $disk = config('skijasi.storage.disk');
        $path = '';
    ...
    public function handleViewFile($file)
    {
        try {
            ...
            $path = public_path('skijasi-images/skijasi.png');
            $mime = mime_content_type($path);
            header("Content-type:$mime");
            ob_clean();
            readfile($path);
