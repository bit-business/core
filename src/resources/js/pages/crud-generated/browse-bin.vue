<template>
  <div>
    <template v-if="!showMaintenancePage">
      <skijasi-breadcrumb-hover full :visibleButtonAction="selected.length != 0">
                      ...
                      >
                        <template v-if="dataRow.browse == 1">
                          ...
                          >
                            ...
                            >
                              <a
                                :href="`${$api.skijasiFile.download(file)}`"
                                target="_blank"
                                >{{ file }}</a
                              >
                            </p>
    ...
    <template v-if="showMaintenancePage">
      ...
      <vs-row v-if="$helper.isAllowedToModifyGeneratedCRUD('browse', dataType)">
        <vs-col vs-lg="12">
          ...
          >
            <img src="/skijasi-images/maintenance.png" alt="Maintenance Icon" />
...
export default {
  ...
  methods: {
    ...
    async getEntity() {
      this.$openLoader();
      try {
        const response = await this.$api.skijasiEntity.browse({
