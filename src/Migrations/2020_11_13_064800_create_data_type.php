...
class CreateDataType extends Migration
{
    ...
    {
        try {
            Schema::create(config('skijasi.database.prefix').'data_types', function (Blueprint $table) {
                $table->id();
            ...
            Schema::create(config('skijasi.database.prefix').'data_rows', function (Blueprint $table) {
                $table->id();
                ...
                $table->foreign('data_type_id')->references('id')->on(config('skijasi.database.prefix').'data_types')->onUpdate('cascade')->onDelete('cascade');
            });
    ...
    {
        Schema::dropIfExists(config('skijasi.database.prefix').'data_rows');
        Schema::dropIfExists(config('skijasi.database.prefix').'data_types');
    }
}
