...
namespace NadzorServera\Skijasi\ContentManager;
...
class ContentGenerator
{
    ...
    $data_type = Skijasi::model('DataType')->where('name', '%s')->first();

                if ($data_type) {
                    Skijasi::model('DataType')->where('name', '%s')->delete();
                }
    TXT;

    /** @var string Menu Insert Statement */
    const MENU_INSERT_STATEMENT = <<<'TXT'
    $menu = Skijasi::model('Menu')->where('key', config('skijasi.default_menu'))->firstOrFail();

                $menu_item = Skijasi::model('MenuItem')
                    ->where('menu_id', $menu->id)
                    ->where('url', '%s')
                    ->first();

                $order = Skijasi::model('MenuItem')->highestOrderMenuItem($menu->id);
    ...
    $menuItem = Skijasi::model('MenuItem')::where('url', '%s');
    ...
    Skijasi::model('Permission')->generateFor('%s');
    TXT;

    const REMOVE_PERMISSIONS_STATEMENT = <<<'TXT'
    Skijasi::model('Permission')->removeFrom('%s');
    TXT;
