...
use NadzorServera\Skijasi\Events\CRUDDataAdded;
use NadzorServera\Skijasi\Events\CRUDDataDeleted;
use NadzorServera\Skijasi\Events\CRUDDataUpdated;
use NadzorServera\Skijasi\Facades\Skijasi;
use NadzorServera\Skijasi\Helpers\ApiDocs;
use NadzorServera\Skijasi\Helpers\ApiResponse;
use NadzorServera\Skijasi\Helpers\DataTypeToComponent;
use NadzorServera\Skijasi\Helpers\WatchTableConfig;
use NadzorServera\Skijasi\Models\DataRow;
...
class SkijasiCRUDController extends Controller
{
    /** @var FileGenerator */
    ...
    public function browse(Request $request)
    {
        try {
            ...
            $protected_tables = Skijasi::getProtectedTables();
            ...
            foreach ($tables as $key => $value) {
                if (! in_array($key, $protected_tables)) {
                    ...
                    $table_with_crud_data['crud_data'] = Skijasi::model('DataType')::where('name', $key)->first();
    ...
    public function read(Request $request)
    {
        try {
            $request->validate([
                'table' => 'required|exists:NadzorServera\Skijasi\Models\DataType,name',
            ]);

            $table = $request->input('table', '');
            $data_type = Skijasi::model('DataType')::where('name', $table)->first();
    ...
    public function edit(Request $request)
    {
        ...
        try {
            $request->validate([
                'id' => 'required|exists:NadzorServera\Skijasi\Models\DataType',
                'name' => [
                    'required',
                    "unique:NadzorServera\Skijasi\Models\DataType,name,{$request->id}",
                    function ($attribute, $value, $fail) {
                        if (! Schema::hasTable($value)) {
                            $fail(__('skijasi::validation.crud.table_not_found', ['table' => $value]));
                        }
                    },
                ],
                'rows' => 'required',
                    ...
                    function ($attribute, $value, $fail) use ($request) {
                        if (! Schema::hasColumn($request->name, $value)) {
                            ...
                            if (! $field_to_relation == 'belongs_to_many') {
                                $request->{$attribute} == $value ? $value : $fail(__('skijasi::validation.crud.table_column_not_found', ['table_column' => "$request->name.{$value}"]));
                            }
                        } else {
                            ...
                            if (! $row['add'] && ! $field['autoincrement'] && $field['notnull'] && is_null($field['default'])) {
                                $fail(__('skijasi::validation.crud.table_column_not_have_default_value', ['table_column' => "$request->name.{$value}"]));
                            } elseif ($row['field'] != 'id' && $field['key'] == 'PRI') {
                                $fail(__('skijasi::validation.crud.id_table_wrong', ['table_column' => "$request->name.{$value}"]));
                            }
                        }
                    },
                ],
                ...
                'create_soft_delete' => ['required', 'boolean', function ($att, $val, $failed) use ($request) {
                    if (isset($request->name) && $val) {
                        if (! Schema::hasColumn($request->name, 'deleted_at')) {
                            $failed(__('skijasi::validation.crud.table_deleted_at_not_exists', [
    ...
    public function add(Request $request)
    {
        ...
        try {
            $request->validate([
                'name' => [
                    'required',
                    'unique:NadzorServera\Skijasi\Models\DataType',
                    function ($attribute, $value, $fail) {
                        if (! Schema::hasTable($value)) {
                            $fail(__('skijasi::validation.crud.table_not_found', ['table' => $value]));
                        }
                ...
                'rows.*.field' => [
                    ...
                    function ($attribute, $value, $fail) use ($request) {
                        if (! Schema::hasColumn($request->name, $value)) {
                            ...
                            $field_to_relation = join('.', $split_attribute);
                            if (! $field_to_relation == 'belongs_to_many') {
                                $fail(__('skijasi::validation.crud.table_column_not_found', ['table_column' => "$request->name.{$value}"]));
                            }
                        } else {
                            $table_fields = SchemaManager::describeTable($request->name);
                            $field = collect($table_fields)->where('field', $value)->first();
    ...
    public function delete(Request $request)
    {
        ...
        try {
            $request->validate([
                'id' => 'required|exists:NadzorServera\Skijasi\Models\DataType,id',
            ]);
    ...
    {
        $menu_key = config('skijasi.default_menu');
