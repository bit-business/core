...
namespace NadzorServera\Skijasi\Tests\Feature;
...
class SkijasiApiCrudManagementTest extends TestCase
{
        ...
        return
            $field_name = [
                ...
                [
                    ...
                    'schema_type' => 'string',
                    'details' => json_encode((object) []),
                    'example' => join(',', ['skijasi', 'test', 'crud', 'management']),
                    'example_update' => join(',', ['skijasi', 'test', 'crud', 'management', 'update']),
                ],
                [
                    'skijasi_type' => 'code',
                    'schema_type' => 'string',
                ...
                [
                    'skijasi_type' => 'relation',
                    'schema_type' => 'bigInteger',
                    'details' => json_encode((object) []),
                    'example' => null,
                ],
                [
                    'skijasi_type' => 'color_picker',
                    'schema_type' => 'string',
                    'details' => json_encode((object) []),
                    'example' => '#000000',
                ...
                [
                    'skijasi_type' => 'upload_image',
                    'schema_type' => 'string',
                    'details' => json_encode((object) []),
                    'example' => 'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619582634819_skijasi.png',
                    'example_update' => 'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619581504968_nadzorservera.png',
                ],
                [
                    'skijasi_type' => 'select_multiple',
                    'schema_type' => 'string',
                    'details' => json_encode([
                        'items' => [
                            ['label' => 'example_1', 'value' => 'example_1'],
                ...
                [
                    'skijasi_type' => 'upload_image_multiple',
                    'schema_type' => 'string',
                    'details' => json_encode((object) []),
                    'example' => "['https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619582634819_skijasi.png','https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619582634819_skijasi.png']",
                    'example_update' => [
                        'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619581504968_nadzorservera.png',
                        'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619581504968_nadzorservera.png',
                    ],
                ],
                [
                    'skijasi_type' => 'upload_file_multiple',
                    'schema_type' => 'string',
                    'details' => json_encode((object) []),
                    'example' => "[\'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619582634819_skijasi.png\',\'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619582634819_skijasi.png\']",
                    'example_update' => "[\'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619581504968_nadzorservera.png\',\'https://skijasi-web.s3-ap-southeast-1.amazonaws.com/files/shares/1619581504968_nadzorservera.png\']",
                ],
            ];
    }
        ...
        for ($index = 1; $index <= $max_count_table_generate; $index++) {
            ...
            if (! Schema::hasTable($table_name)) {
                Schema::create($table_name, function (Blueprint $table) use ($index, $table_names) {
                    ...
                    foreach ($this->getFields() as $key => ['skijasi_type' => $skijasi_type, 'schema_type' => $schema_type]) {
                        if ($skijasi_type == 'relation') {
                            if ($index >= 2) {
                                $table_name_relation = $table_names[0];
                                $table->{$schema_type}($skijasi_type)->nullable()->unsigned();

                                $table->foreign($skijasi_type)->references('id')->on($table_name_relation)->onDelete('cascade');
                            }
                        } else {
                            $table->{$schema_type}($skijasi_type)->nullable();
                        }
                    }
                    $table->softDeletes();
                    $table->timestamps();
    ...
    public function testBrowseCrudManagement()
    {
        ...
        $expect_table = config('skijasi-watch-tables');
    ...
    public function testAddCrudManagement()
    {
        ...
        foreach ($table_names as $index_table_name => $table_name) {
            ...
            foreach ($const_fields as $key => ['skijasi_type' => $skijasi_type, 'schema_type' => $schema_type, 'details' => $details]) {
                if ($index_table_name == 0 && $skijasi_type == 'relation') {
                    continue;
                }

                $field_name = ucwords(str_replace(['_'], ' ', $skijasi_type));
                $row = [
                    'field' => $skijasi_type,
                    'type' => $skijasi_type,
                ...
                if ($skijasi_type == 'relation') {
                    $destination_field['skijasi_type'] = 'id';
                    $destination_more_field['skijasi_type'] = ['select_multiple'];
                    $row['relationType'] = ['has_one', 'has_many'][rand(0, 1)];
                    $row['destinationTable'] = $table_names[0];
                    $row['destinationTableColumn'] = $destination_field['skijasi_type'];
                    $row['destinationTableDisplayColumn'] = $destination_field['skijasi_type'];
                    $row['destinationTableDisplayMoreColumn'] = $destination_more_field['skijasi_type'];
                    $row['required'] = false;
                }
            ...
            if (rand(0, 1)) {
                ...
                class {$controller_name} extends \NadzorServera\Skijasi\Controllers\SkijasiBaseController {}
                PHP;
    ...
    public function testAddEditEntityCrudManagement()
    {
        ...
        foreach ($this->getFields() as $key => $value) {
            $fields[$value['skijasi_type']] = $value;
        }
