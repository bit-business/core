  ...
  <vs-col :vs-lg="size" vs-xs="12" class="skijasi-switch__container">
    <div v-if="label == 'Maintenance for all pages.' && status">
      <label v-if="label != ''" for="" class="skijasi-switch__label"
        >{{ label }} <span style="color: red;">({{ $t("site.maintenanceMode")}} <a href="https://skijasi-docs.uatech.co.id/getting-started/configuration">{{ $t("database.browse.fieldNotSupport.button.visitDocs")}}</a>)</span>
